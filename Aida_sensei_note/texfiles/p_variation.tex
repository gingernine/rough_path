...
\begin{prf}
	...
	が従うことを示せばよい．いま，$M \coloneqq \sup{u \in [s,t]}{|f(x_u)|} < \infty$
	とおけば
	\begin{align}
	...
	\end{align}
	が成り立つ．$\Norm{x^n - x}{C^1} \longrightarrow 0$の仮定より
	$(x^n)_{n=1}^\infty$及び$x$の値域は或るコンパクト集合$K$に含まれるから，
	$K$上での$f$の一様連続性より任意の$\epsilon > 0$に対し或る$\epsilon > \delta > 0$が存在して
	$v,w \in K,\ |v - w| < \delta$なら$|f(v) - f(w)| < \epsilon$
	が満たされる．すなわち$\Norm{x^n - x}{C^1} < \delta$なら
	\begin{align}
		\sup{u \in [s,t]}{\left|f(x^n_u) - f(x_u) \right|} < \epsilon
	\end{align}
	が成立する．$\Norm{x^n - x}{C^1} \longrightarrow 0$より，
...
\begin{screen}
	\begin{dfn}[$p$-variation]
		$(V,\Norm{\cdot}{})$をノルム空間とし，
		$[0,T]$上の$V$値写像$x$と$[s,t] \subset [0,T]$に対して$p$-variation $(p > 0)$を次で定める:
		\begin{align}
			\Norm{x}{p,[s,t]}
		...
		また$p \geq 1$として，線型空間$B_{p,T}(V)$を
		\begin{align}
			B_{p,T}(V)
...
次の結果によれば，$0 < p < 1$に対し$B_{p,T}(V)$を定めても
零写像のみの空間でしかない．
\begin{screen}
	\begin{thm}[$0 < p < 1$に対して有界$p$-variationなら定数]\label{thm:p_variation_0_1}
		$x:[0,T] \longrightarrow V$を連続写像とする．
		このとき，$p \in (0, 1)$に対し$\Norm{x}{p} < \infty$が成り立つなら$x$は定数写像である．
	\end{thm}
\end{screen}
...
\begin{screen}
	\begin{thm}[$p$-variationの$p$に関する単調減少性]\label{thm:p_variation_monotonously_decreasing}
		$V$をノルム空間とするとき，$x:[0,T] \longrightarrow V$に対して
		$1 \leq p \leq q$なら$\Norm{x}{p} \geq \Norm{x}{q}$が成立する．
		特に$B_{p,T}(V) \subset B_{q,T}(V)$が従う．
	\end{thm}
\end{screen}

\begin{prf}
	$a,b \geq 0,\ r \geq 1$に対し$a^r + b^r \leq (a+b)^r$が成り立つから
	\begin{align}
		\left[ \sum_{i=1}^n a_i^r \right]^{1/r} 
		\leq \sum_{i=1}^n a_i,
		\quad (a_i \geq 0,\ n \geq 1,\ r \geq 1)
	\end{align}
	を得る．従って任意の$x:[0,T] \longrightarrow V$と$D \in \delta[0,T]$に対し
	\begin{align}
		\left[ \sum_D \left( \Norm{x_{t_i} - x_{t_{i-1}}}{}^p \right)^{q/p} \right]^{p/q}
		\leq \sum_D \Norm{x_{t_i} - x_{t_{i-1}}}{}^p
	\end{align}
	が満たされ$\Norm{x}{q} \leq \Norm{x}{p}$が成立する．
	\QED
\end{prf}
	...
	\begin{description}
		...
		\item[第二段] $\Norm{x^n - x}{p} \longrightarrow 0\ (n \longrightarrow \infty)$を示す．
			...
			\begin{align}
				...
				\quad (n,m > n_\epsilon)
			\end{align}
			が成り立っている．$D$は有限分割であるから，$m \longrightarrow \infty$として
			\begin{align}
