	...
	\begin{prf}
		...
		\begin{description}
			...
			\item[第五段]
				...
				$D_1 \coloneqq [s,t] \cap D$とおいて
				$D_1 = \{t_0 < \cdots < t_k\}$と表されているとする．
				このとき$D_2 \coloneqq D \backslash \left( D_1 \cup \{ t_{k+1} \} \right)$として
				\begin{align}
					\omega(s,t + h) - \epsilon
				...
				\end{align}
				が成り立つ．$\psi$の(一様)連続性より$\Norm{\psi_{t_k,t_{k+1}}}{}^p \longrightarrow \Norm{\psi_{t_k,t}}{}^p\ (h \longrightarrow +0)$が成り立つから
				\begin{align}
			...
			\item[第六段]
				...
				\end{align}
				が成り立ち，値の差は上述した連続性により縮まる．また
				$|\omega(s,t) - \omega(s-h,t+k)|$については，
				$h,k \longrightarrow +0$として$\omega(s-h,t+k)$は単調減少に$\omega(s,t)$に近づく．
				単調性より極限は$h,k$の近づけ方に依らず
				\begin{align}
					\omega(s,t) = \lim_{h \to +0} \lim_{k \to +0} \omega(s-h,t+k) = \lim_{h,k \to +0} \omega(s-h,t+k)
				\end{align}
				となり，同様に
				\begin{align}
					\omega(s,t) = \lim_{h \to +0} \lim_{k \to +0} \omega(s+h,t-k) = \lim_{h,k \to +0} \omega(s+h,t-k)
				\end{align}
				も得られ，$\omega(s-h,t-k)$は$h,k \to 0\ $(近づけ方に依らない)として$\omega(s,t)$に収束し
				$\omega$の連続性が出る．
				\QED
		\end{description}
	\end{prf}
