	...
	\begin{prf}\mbox{}
		\begin{description}
			...
			\item[第二段]
				...
				\begin{align}
					\Delta_T \ni (s,t) \longmapsto M \Norm{X^1}{1,[s,t]}
				\end{align}
				はコントロール関数であるから，
				\begin{align}
				...
				\end{align}
				が成立し$(s,s)$での$X^2$の連続性を得る．
			
			\item[第三段]
				$s < t$を満たす点$(s,t) \in \Delta_T$において$X^2$が連続であること示す．
				$[s,t] \cap [a,b] \neq \emptyset$を満たす程十分近い点
				\footnote{
					点$(a,b)$を点$(s,t)$に近づけるとき$\compcrossnorm{X^2_{s,t} - X^2_{a,b}}{2}$
					がいくらでも小さくなることを示したい．
				}
				$(a,b) \in \Delta_T$を取る．
				(\refeq{eq:signature_of_path_1})より，
				任意の$\epsilon > 0$に対して或る$\eta > 0$が存在し，
				$D_1 \in \delta[s,t]$と$D_2 \in \delta[a,b]$
				が$|D_1|,|D_2| < \eta$を満たす限り
				\begin{align}
					\compcrossnorm{X^2_{s,t} - \sum_{D_1} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i}}{2} < \frac{\epsilon}{3},
					\quad \compcrossnorm{X^2_{a,b} - \sum_{D_2} X^1_{a,v_{j-1}} \otimes_{1,2} X^1_{v_{j-1},v_j}}{2} < \frac{\epsilon}{3}
				\end{align}
				が成立する．このとき$D_3 \coloneqq \{s,t,a,b\} \cup D_1 \cup D_2$とおいて
				\begin{align}
					D_4 \coloneqq D_3 \cap [s,t] \cap [a,b],
					\quad D'_1 \coloneqq D_1 \cup D_4,
					\quad D'_2 \coloneqq D_2 \cup D_4
				\end{align}
				を定めると，
		\end{description}
