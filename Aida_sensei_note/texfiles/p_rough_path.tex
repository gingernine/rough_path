\section{The notion of rough path}
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段]
				...
				\end{align}
				が成立し(\refeq{eq:thm_otimes_is_a_multiplication_1})が得られる．
	...
	\begin{align}
		a \otimes b \coloneqq
		\Biggl(\sum_{j=0}^{k} a_j \otimes_{j,k} b_{k-j} \Biggr)_{k=0}^n,
		\quad a,b \in T^{(n)}(V)
	\end{align}
	...
	\begin{prf}完備性を示す．
		\begin{description}
			...
			より$X$は有限$p$-変動となり$X \in \Omega_p(V)$が従う．任意の$0 \leq i \leq [p]$
