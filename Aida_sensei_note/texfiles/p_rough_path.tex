\section{The notion of rough path}
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段]
				...
				\begin{align}
					M \coloneqq \sup{(x,y) \in \Delta_T}{\compcrossnorm{Y_{x,y}}{1}}
				\end{align}
			...
			\item[第三段]
				...
				\begin{description}
					\item[(1)] (\refeq{map:lem_signature_of_path_1})がコントロール関数であるから，
						或る$\eta_1 > 0$が存在して$|s-a|,|t-b| < \eta_1$ならば
						\begin{align}
							\Norm{X^1}{1,[s \wedge a,s \vee a]},\ 
							\Norm{X^1}{1,[t \wedge b,t \vee b]} < \epsilon
						\end{align}
						が満たされる．
						
					\item[(2)] 或る$\eta_2 > 0$が存在して$|s-a|,|t-b| < \eta_2$ならば
						$[s,t] \cap [a,b] \neq \emptyset$が満たされる．
						
					\item[(3)] $Y$は$\Delta_T$上一様連続であるから，
						或る$\eta_3 > 0$が存在して$|s-a| < \eta_3$なら
						\begin{align}
							\compcrossnorm{Y_{s,u} - Y_{a,u}}{k} < \epsilon,
							\quad (s,a \leq \forall u \leq T)
						\end{align}
						が満たされる．
						
					\item[(4)] 補題\ref{lem:def_integration_of_continuous_mapping_by_X_1}
	...
	\begin{screen}
		\begin{dfn}[パスのシグネチャー]
			有界変動な連続写像$x:[0,T] \longrightarrow V = V^{\otimes 1}$に対して
			$X^1_{s,t} \coloneqq x_t-x_s,\ (\forall (s,t) \in \Delta_T)$とおけば，
			補題\ref{lem:def_integration_of_continuous_mapping_by_X_1}より逐次的に
			次を構成する:
			\begin{align}
	...
	$S(x)_{[s,t]}$の最初の$n+1$成分を抜き取り
	\begin{align}
		S_n(x): \Delta_T \ni (s,t) \longmapsto S_n(x)_{[s,t]} \coloneqq
		(1,X^1_{s,t},\cdots,X^n_{s,t})
		\in T^{(n)}(V)
	\end{align}
	とおく．
	
	\begin{screen}
		\begin{thm}[$S_n(x)$は$n$次乗法的]
			$S_n(x):\Delta_T \longrightarrow T^{(n)}(V)$
			は$n$次乗法的汎関数である．
		\end{thm}
	\end{screen}
	...
	\begin{prf}
		\begin{description}
			\item[第四段]
				...
				$u$を節点に含む$[s,t]$の細分列$\left(D_n\right)_{n=1}^\infty,
				\ (|D_n| \longrightarrow 0)$を取り，
				$u$を境に
				\begin{align}
					D_n = D^1_n + D^2_n,
					\quad D_n &= \{s=u^n_0 < \cdots < u < \cdots < u^n_{m_n} = t\}, \\
					D^1_n &= \{s=u^n_0 < \cdots < u\},
					\quad D^2_n = \{u < \cdots < u^n_{m_n} = t\}
				\end{align}
				と分割する．このとき
				\begin{align}
					&\compcrossnorm{X^2_{s,t} - \left( X^2_{s,u} + X^1_{s,u} \otimes_{1,2} X^1_{u,t} + X^2_{u,t} \right)}{2} \\
					&\leq \compcrossnorm{X^2_{s,t} - \sum_{D_n} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i}}{2} \\
						&+ \compcrossnorm{\sum_{D^1_n} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i} 
						+ \sum_{D^2_n} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i}
						- \left( X^2_{s,u} + X^1_{s,u} \otimes_{1,2} X^1_{u,t} + X^2_{u,t} \right)}{2} \\
					&\leq \compcrossnorm{X^2_{s,t} - \sum_{D_n} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i}}{2} \\
						&+ \compcrossnorm{\sum_{D^1_n} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i} - X^2_{s,u}}{2} \\
						&+ \compcrossnorm{\sum_{D^2_n} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i}
						- \left( X^1_{s,u} \otimes_{1,2} X^1_{u,t} + X^2_{u,t} \right)}{2} \\
					&\leq \compcrossnorm{X^2_{s,t} - \sum_{D_n} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i}}{2} \\
						&+ \compcrossnorm{\sum_{D^1_n} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i} - X^2_{s,u}}{2} \\
						&+ \compcrossnorm{\sum_{D^2_n} \left( X^1_{s,u_{i-1}} - X^1_{u,u_{i-1}} \right) \otimes_{1,2} X^1_{u_{i-1},u_i} + \sum_{D^2_n} X^1_{u,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i}
						- \left( X^1_{s,u} \otimes_{1,2} X^1_{u,t} + X^2_{u,t} \right)}{2} \\
					&= \compcrossnorm{X^2_{s,t} - \sum_{D_n} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i}}{2} \\
						&+ \compcrossnorm{\sum_{D^1_n} X^1_{s,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i} - X^2_{s,u}}{2} \\
						&+ \compcrossnorm{\sum_{D^2_n} X^1_{u,u_{i-1}} \otimes_{1,2} X^1_{u_{i-1},u_i}
						- X^2_{u,t}}{2}
				\end{align}
		\end{description}	
	\end{prf}
