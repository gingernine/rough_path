\section{The notion of rough path}
	...
	$\varphi_{j,k}$は$V^{\otimes j} \times V^{\otimes k-j}$上の或る唯一の双線型
	...
	\begin{screen}
		\begin{dfn}[finite $p$-variation]
			...
			\begin{align}
				\compcrossnorm{X^i_{s,t}}{k} \leq \omega(s,t)^{i/p},
				\quad (\forall i=1,\cdots,n,\ \forall (s,t) \in \Delta_T)
				\label{eq:def_finite_p_variation}
			\end{align}
			を満たすとき，$X$は有限$p$-変動(finite $p$-variation)を持つという．
		\end{dfn}
	\end{screen}
	
	(\refeq{eq:def_finite_p_variation})を満たす$\omega$が存在する場合，
	$p \geq n$ならば$\Norm{X}{p} < \infty$が成り立つ．実際，
	\begin{align}
		\sum_D \compcrossnorm{X^i_{s,t}}{k}^{p/i} \leq \omega(0,T),
		\quad (\forall D \in \delta[0,T],\ i=1,\cdots,n)
	\end{align}
	となり$\Norm{X^i}{p/i} < \infty\ (i=1,\cdots,n)$が従い，
	$p/i \geq 1\ (i=1,\cdots,n)$だから
	定理\ref{thm:p_variation_monotonously_decreasing_2}により
	\begin{align}
		\Norm{X^i}{p} \leq \Norm{X^i}{p/i}\ (i=1,\cdots,n)
	\end{align}
	が満たされる．よって任意の$D \in \delta[0,T]$に対して
	\begin{align}
		\sum_D \compcrossnorm{X_{t_{i-1},t_i}}{}^p
		= \sum_D \left( \compcrossnorm{X^0_{t_{i-1},t_i}}{1}^p +  \right)
	\end{align}
	
	\begin{screen}
		\begin{dfn}[finite total $p$-variation]
			$p \geq 1$とする．$X \in C_0 \left(\Delta_T,T^{(n)}(V) \right)$
			が有限総$p$-変動(finite total $p$-variation)を持つとは，任意の$1 \leq i \leq n$に対して
			\begin{align}
				\Norm{X^i}{p/i} < \infty
			\end{align}
			が満たされることをいう．また次を定める:
