\section{The notion of rough path}
	...
	また$k=0,1$に対しては$V^{\otimes 0} = \R,\ V^{\otimes 1} = V$とする．
	...
	\begin{prf}完備性を示す．
		...
		\item[第二段] $X$が Chen's identity を満たすことを示す．
			...
			すなわち$X$は$p$-ラフパスであり，(\refeq{eq:thm_p_rough_path_complete_dist_2})より
			$d_p(X^k,X) \longrightarrow 0\ (k \longrightarrow \infty)$が出る．
			\QED
	\end{description}
\end{prf}