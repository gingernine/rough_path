\section{2018/06/15 課題}
	...
	\begin{prf}
		...
		$\bar{x}$の連続性より
		\begin{align}
			\Set{\bar{x}_{t_{i-1}} + \theta_n(\bar{x}_u - \bar{x}_{t_{i-1}})}{u \in [t_{i-1},t_i],\ t_{i-1},t_i \in D,\ 0 \leq \theta_n \leq 1,\ D \in \delta[s,t]} \subset K
		\end{align}
		を満たすコンパクト集合$K \subset \R$が存在するから，
		\begin{align}
			M \coloneqq \sup{x \in K}{\left| f^{(n)}(x) \right|}
		\end{align}
		とおけば
		\begin{align}
		...
		\end{align}
		が成立する．補題\ref{lem:signature_of_path}より
		\begin{align}
			v \coloneqq \sup{D \in \delta[s,t]}{\sum_D \int_{t_{i-1}}^{t_i} \left| \bar{x}_u - \bar{x}_{t_{i-1}} \right|^{n-1}\ du} < \infty
		\end{align}
		が満たされ，$\bar{x}$の一様連続性より
		\begin{align}
			&\left| \sum_D \int_{t_{i-1}}^{t_i} \int_0^1 \int_0^{\theta_1} \cdots \int_0^{\theta_{n-1}} f^{(n)}(\bar{x}_{t_{i-1}} + \theta_n(\bar{x}_u - \bar{x}_{t_{i-1}}))(\bar{x}_u - \bar{x}_{t_{i-1}})^n\ d\theta_n\cdots d\theta_2\ d\theta_1\ dx_u \right| \\
			&\qquad \leq \frac{1}{n!} M \Norm{x}{C^1} v \sup{\substack{t_{i-1} \leq u \leq t_i \\ t_{i-1},t_i \in D}}{\left| \bar{x}_u - \bar{x}_{t_{i-1}} \right|}
			\longrightarrow 0
			\quad (|D| \longrightarrow 0)
		\end{align}
		となる．
