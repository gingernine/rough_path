\section{クロスノルム}
	...
	\begin{screen}
		\begin{dfn}[クロスノルム]\label{def:cross_norm}
			...
			\begin{align}
				\alpha(x_1 \otimes \cdots \otimes x_n) &\leq \Norm{x_1}{X_1} \Norm{x_2}{X_2} \cdots \Norm{x_n}{X_n}, && (\forall x_i \in X_i,\ i=1,\cdots,n), \label{eq:cross_norm_def_1}\\
				...
				&& (\forall x_i^* \in X_i^*,\ i=1,\cdots,n) \label{eq:cross_norm_def_2}
			\end{align}
			を満たすようなノルム$\alpha:\bigotimes_{i=1}^n X_i \longrightarrow [0,\infty)$を
			クロスノルム(cross norm)と呼ぶ\footnotemark．
		\end{dfn}
	\end{screen}
	\footnotetext{
		\cite{key8}ではこの二性質を満たすものをリーゾナブルクロスノルムと分類するが，
		本稿ではクロスノルムと書くだけにする．
	}
	\begin{screen}
		\begin{thm}
			$\K$-Banach空間の族$(X_i)_{i=1}^{n}$のテンソル積
	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段]
				...
				を用いて
				\begin{align}
					v = \sum_{j=1}^{m} c_j u^1_{\lambda^j_1} \otimes \cdots \otimes u^n_{\lambda^j_n},
					\quad (c_j \neq 0,\ j=1,\cdots,m)
				\end{align}
				と表現できる．
				$u^1_{\lambda^1_1}$について，
				もし全ての$2 \leq j \leq m$に対し$u^1_{\lambda^j_1} = u^1_{\lambda^1_1}$が満たされているなら，
				$\hat{x}_1^* \in X_1^*$を
				\begin{align}
					\inprod<u^1_{\lambda^1_1}, \hat{x}_1^*> = \Norm{u^1_{\lambda^1_1}}{X_1},
					\quad \Norm{\hat{x}_1^*}{X^*_1} = 1
				\end{align}
				を満たすように選ぶ(Hahn-Banachの定理)．$u^1_{\lambda^j_1} \neq u^1_{\lambda^1_1}$を満たす$j$がある場合，
				\begin{align}
					L_1 \coloneqq \Span{\Set{u^1_{\lambda^j_1}}{2 \leq j \leq m,\ u^1_{\lambda^j_1} \neq u^1_{\lambda^1_1}}}
				\end{align}
				により閉部分空間を定めれば$u^1_{\lambda^1_1}$と$L_1$との距離$d_1$は正であり，Hahn-Banachの定理より
				\begin{align}
					\inprod<x_1,\hat{x}_1^*> = 0\ (\forall x_1 \in L_1),
					\quad \inprod<u^1_{\lambda^1_1},\hat{x}_1^*> = d_1 > 0,
					\quad \Norm{\hat{x}_1^*}{X_1^*} = 1
				\end{align}
				を満たす$\hat{x}_1^* \in X_1^*$を取ることができる．
				同様に$\hat{x}_i^* \in X_i^*\ (i=2,\cdots,n)$を選べば
				\begin{align}
					\hat{x}_1^* \otimes \cdots \otimes \hat{x}_n^* \left(u^1_{\lambda^j_1} \otimes \cdots \otimes u^n_{\lambda^j_n} \right) =
					\begin{cases}
						\hat{x}_1^* \otimes \cdots \otimes \hat{x}_n^* \left( u^1_{\lambda^1_1} \otimes \cdots \otimes u^n_{\lambda^1_n} \right), & \left(u^i_{\lambda^j_i} = u^i_{\lambda^1_i},\ i=1,\cdots,n \right), \\
						0, & (\mbox{o.w.}),
					\end{cases}
				\end{align}
				$(j=2,\cdots,m)$が満たされるから
				\begin{align}
					0 < |c_1| \hat{x}_1^* \otimes \cdots \otimes \hat{x}_n^* \left( u^1_{\lambda^1_1} \otimes \cdots \otimes u^n_{\lambda^1_n} \right)
		...
		\begin{description}
			\item[第一段]
				...
				\begin{align}
					...
					\quad (\forall v \in \bigotimes_{i=1}^n X_i).
				\end{align}
				いま，任意に$v \in \bigotimes_{i=1}^n X_i$を取れば，$v$の任意の分割
				\begin{align}
					v = \sum_{j=1}^{m} x^j_1 \otimes \cdots \otimes x^j_n.
				\end{align}
				に対し
				\begin{align}
					\pi(v) \leq \sum_{j=1}^m \pi \left( x^j_1 \otimes \cdots \otimes x^j_n \right)
					= \sum_{j=1}^m \Norm{x^j_1}{X_1} \cdots \Norm{x^j_n}{X_n}
				\end{align}
				が満たされ$\pi \leq \lambda$が従う．
				よって$\lambda$が式(\refeq{eq:cross_norm_def_1})を満たせば，
				定理\ref{thm:projective_norm_is_maximum_cross_norm}より$\lambda = \pi$が得られる．
