	...
	\begin{prf}\mbox{}
		\begin{description}
			\item[第一段]
				...
				\end{align}
				が成立する．以下，数段にわたり$\left( u^1_{\lambda_1} \otimes 
				\cdots \otimes u^n_{\lambda_n} \right)_{\lambda_1,\cdots,\lambda_n}$
				の一次独立性を示す．
				
			\item[第二段]
				いま，各$u^i_{\lambda_i}$の生成する一次元空間を
				$U^i_{\lambda_i}$と表し
				\begin{align}
					V_i \coloneqq \bigoplus_{\lambda_i \in \Lambda_i} U^i_{\lambda_i},
					\quad (i=1,\cdots,n)
				\end{align}
				とおく．このとき，
				\begin{align}
					\bigotimes_{i=1}^n E_i 
					\cong \bigotimes_{i=1}^n V_i
					\cong \bigoplus_{\lambda_1,\cdots,\lambda_n} U^1_{\lambda_1} \otimes \cdots \otimes U^n_{\lambda_n}
				\end{align}
				が成立することを示す．
			...
			\item[第三段]
				第一の同型を定める写像$f$を構成する．
				$\left( u^i_{\lambda_i} \right)_{\lambda_i \in \Lambda_i}$
				は$E_i$の基底であるから，任意の$e_i \in E_i$は
				\begin{align}
					e_i = \sum_{\substack{\lambda_i \in \Lambda_i \\ c_{\lambda_i}(e_i) \neq 0}}
					c_{\lambda_i}(e_i) u^i_{\lambda_i},
					\quad \left( c_{\lambda_i}(e_i) \in \K,\ (\forall \lambda_i \in \Lambda_i) \right)
				\end{align}
				と一意に表現できる．従って，$e_i$に対応する$V_i$の元$w_i(e_i)$を
				\begin{align}
					w_i(e_i):\Lambda_i \ni \lambda_i \longmapsto c_{\lambda_i}(e_i) u^i_{\lambda_i}
				\end{align}
				により定めれば，
				$w_i(e_i) = w_i(e'_i)$ならば$e_i = e'_i$が満たされるから
				\begin{align}
					f_i:E_i \ni e_i \longmapsto w_i(e_i) \in V_i
				\end{align}
				...
				\begin{align}
					...
					&= f^{-1}_1 \otimes \cdots \otimes f^{-1}_n
						...
						&= (e_1 \otimes \cdots \otimes e_n),
						\quad \biggl(\forall e_i \otimes \cdots \otimes e_n \in \bigotimes_{i=1}^n E_i\biggr) \\
					...
					&= f_1 \otimes \cdots \otimes f_n
						...
						&= (v_1 \otimes \cdots \otimes v_n),
						\quad \biggl(\forall v_i \otimes \cdots \otimes v_n \in \bigotimes_{i=1}^n V_i\biggr)
				\end{align}
				が成立し，式(\refeq{eq:thm_tensor_product_is_bilinear})より
				$f^{-1}_1 \otimes \cdots \otimes f^{-1}_n \circ 
				f_1 \otimes \cdots \otimes f_n$も
				$f_1 \otimes \cdots \otimes f_n \circ 
				f^{-1}_1 \otimes \cdots \otimes f^{-1}_n$
				もそれぞれ$\bigotimes_{i=1}^n E_i,\bigotimes_{i=1}^n V_i$上の恒等写像に一致するので
				$(f_1 \otimes \cdots \otimes f_n)^{-1} = f^{-1}_1 \otimes \cdots \otimes f^{-1}_n$
				を得る．ここで$f \coloneqq f_1 \otimes \cdots \otimes f_n$とする．
				
			\item[第四段]
				第二の同型を定める写像$g$を構成する．
				\begin{align}
					\bigoplus_{i=1}^n V_i \ni (v_1,\cdots,v_n) \longmapsto \left( v_1(\lambda_1)\otimes\cdots\otimes v_n(\lambda_n) \right)_{\lambda_1,\cdots,\lambda_n}
					\in \bigoplus_{\lambda_1,\cdots,\lambda_n} U^1_{\lambda_1} \otimes \cdots \otimes U^n_{\lambda_n}
				\end{align}
				は$n$重線型写像であるから，普遍性より或る$g \in \Hom{\bigotimes_{i=1}^n V_i}{\bigoplus_{\lambda_1,\cdots,\lambda_n} U^1_{\lambda_1} \otimes \cdots \otimes U^n_{\lambda_n}}$
				が唯一つ存在して
				\begin{align}
					g(v_1 \otimes \cdots \otimes v_n)
					= \left( v_1(\lambda_1)\otimes\cdots\otimes v_n(\lambda_n) \right)_{\lambda_1,\cdots,\lambda_n},
					\quad (\forall v_i \in V_i,\ \forall i=1,\cdots,n)
				\end{align}
				を満たす．これが求める線型同型である．以下，$g$の逆写像$h$の存在を示す．
				\begin{align}
					\iota_{\lambda_i}:U^i_{\lambda_i}
					\longrightarrow V_i,
					\quad (\lambda_i \in \Lambda_i,\ i=1,\cdots,n)
				\end{align}
				を，次を満たす標準単射として定める:
				\begin{align}
					\left(\iota_{\lambda_i}(u)\right)(\lambda)
					= \begin{cases}
						u, & (\lambda = \lambda_i), \\
						0, & (\lambda \neq \lambda_i),
					\end{cases}
					\quad (\lambda \in \Lambda_i,\ u \in U^i_{\lambda_i}).
				\end{align}
				$\iota_{\lambda_i}$は線型であるから
				$\iota_{\lambda_1} \otimes \cdots \otimes \iota_{\lambda_n}:
				U^1_{\lambda_1} \otimes \cdots \otimes U^n_{\lambda_n} \longrightarrow
