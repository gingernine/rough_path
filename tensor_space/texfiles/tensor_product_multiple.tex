\section{一時的}
	\begin{screen}
		\begin{thm}[可測関数列の各点極限の可測性]
			$E$を位相空間とし，$C(E,\R)$により連続写像$E \longrightarrow \R$の全体を表す．
			\begin{align}
				\borel{E} = \sgmalg{\bigcup_{g \in C(E,\R)} \Set{g^{-1}(B)}{B \in \borel{\R}}}
			\end{align}
			が満たされている場合，可測空間$(X,\mathcal{M})$上の$E$-値可測関数列
			$(f_n)_{n=1}^{\infty}$が各点で収束しているなら，
...
\section{テンソル積}
	...
	\begin{screen}
		...
		\label{thm:associativity_of_tensor_products}
			$(E_i)_{i=1}^n$を$\K$-線型空間の族とする．
			任意の$k=1,\cdots,n-1$に対し，
			\begin{align}
				\bigotimes_{i=1}^n E_i
				=\biggl( \bigotimes_{i=1}^k E_i \biggr) \bigotimes \biggl( \bigotimes_{i=k+1}^n E_i \biggr)
			\end{align}
			が成立する．
		\end{thm}
	\end{screen}
